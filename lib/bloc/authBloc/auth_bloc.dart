...
class AuthBloc extends Bloc<AuthEvent, AuthenticateState> {
  ...
  AuthBloc(this.userRepository)
      : super(AuthenticateState(eventState: EventState.INITIAL, error: '')) {
    ...
    on<UpdatePicture>((event, emit) async {
      try {
        ...
        bool update =
            await userRepository.updatePhoto(event.image, event.listUsers);
        if (update) {
          ListUsers listUsers = await userRepository.signIn(
              event.listUsers.data[0].attributes.email,
              event.listUsers.data[0].attributes.password);
          emit(AuthenticateState(
            listUsers: listUsers,
            eventState: EventState.LOADED,
            error: '',
    ...
    on<UpdateInfos>((event, emit) async {
      emit(AuthenticateState(
          ...
          error: 'LOADING'));
      try {
        bool update = await userRepository.updateUserInfos(
            event.id, event.fullname, event.password);
        if (update) {
          ListUsers listUsers =
