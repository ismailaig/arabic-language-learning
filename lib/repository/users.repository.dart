...
class UserRepository {
  Future<ListUsers> signIn(String email, String password) async {
    String url =
        "https://arabic-language.herokuapp.com/api/appusers?populate=*&filters[email][\$eq]=$email&filters[password][\$eq]=$password";
    try {
  ...
  Future<bool> deletePhoto(ListUsers listUsers) async {
    String url =
        "https://arabic-language.herokuapp.com/api/upload/files/${listUsers.data[0].attributes.photo.data!.id}";
    try {
